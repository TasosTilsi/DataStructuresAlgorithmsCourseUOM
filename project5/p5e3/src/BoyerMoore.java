...
public class BoyerMoore {
    ...
    public void printAll(String txt) {
        int[] offsets = Arrays.stream(searchAll(txt)).distinct().sorted().toArray();
        int loopCount=0;
        StringBuilder s = new StringBuilder();
        if (offsets.length != 0) {

            StdOut.println("Number of occurrences: " + (offsets.length - 1));
            StdOut.println(txt);
            for (int i = 1; i < offsets.length; i++) {
                loopCount = offsets[i] - s.toString().length();
                for (int j = 0; j < loopCount; j++) {
                    s.append(" ");
                }
                if (offsets[i] != -1) {
                    s.append(String.format("%s", pat));
                }

            }
            StdOut.println(s);
        } else {
            StdOut.println("Number of occurrences: 0");
        }
    ...
    public static void main(String[] args) {
        ...
        int[] offset3 = boyermoore1.searchAll(txt);
        System.out.println(Arrays.toString(offset3));
        System.out.println(Arrays.toString(Arrays.stream(offset3).distinct().sorted().toArray()));

        boyermoore1.printAll(txt);
//        boyermoore1.print(txt);
